...
import { Eye, GitCompareArrows, Wand2, TrendingUp, Star, ArrowRight, Check, Loader2 } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl px-6 pt-20 pb-12 md:pt-28 md:pb-20">
        ...
        >
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <Link to="/sign-up" className="group inline-flex items-center gap-2 rounded-lg bg-foreground px-5 py-2.5 text-sm font-medium text-background shadow-lg transition hover:opacity-90">
              {t.hero.ctaPrimary}
              <ArrowRight className="h-4 w-4 transition group-hover:translate-x-0.5" />
            </Link>
            <a href="#solution" className="inline-flex items-center gap-2 rounded-lg border border-border bg-background/50 px-5 py-2.5 text-sm font-medium backdrop-blur transition hover:bg-muted">
              {t.hero.ctaSecondary}
            </a>
          </div>
...
export function Logos() {
  const { t } = useApp();
  const names = ["NORTHSTAR", "PARALLAX", "MERIDIAN", "AXIOM", "VERSO", "HELIO"];
  return (
    <section className="border-b border-border py-12">
      <div className="mx-auto max-w-7xl px-6">
        <p className="text-center text-xs uppercase tracking-wider text-muted-foreground">{t.logos}</p>
        <div className="mt-6 grid grid-cols-2 gap-x-8 gap-y-6 opacity-60 sm:grid-cols-3 md:grid-cols-6">
          {names.map((n) => (
            <div key={n} className="text-center font-display text-sm font-semibold tracking-[0.2em] text-muted-foreground">{n}</div>
          ))}
        </div>
      </div>
    </section>
...
export function Stats() {
  ...
  return (
    <section id="stats" className="border-b border-border py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-6">
        <h2 className="font-display max-w-2xl text-balance text-3xl font-semibold tracking-tight md:text-4xl">{t.stats.title}</h2>
        <div className="mt-12 grid grid-cols-2 gap-px overflow-hidden rounded-2xl border border-border bg-border md:grid-cols-4">
          {items.map((it, i) => (
            ...
            >
              <div className="font-display text-4xl font-semibold tabular-nums tracking-tight md:text-5xl">{it.v}</div>
    ...
    <section className="border-b border-border py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-6">
        <div className="max-w-2xl">
          <div className="text-xs uppercase tracking-wider text-accent">{t.problems.eyebrow}</div>
          <h2 className="font-display mt-3 text-balance text-3xl font-semibold tracking-tight md:text-4xl">{t.problems.title}</h2>
...
export function Solution() {
  ...
  return (
    <section id="solution" className="border-b border-border py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-6">
        <div className="max-w-2xl">
          <div className="text-xs uppercase tracking-wider text-accent">{t.solution.eyebrow}</div>
          <h2 className="font-display mt-3 text-balance text-3xl font-semibold tracking-tight md:text-4xl">{t.solution.title}</h2>
            ...
            return (
              ...
              >
                ...
                <div className="relative">
                  ...
                  <h3 className="font-display mt-5 text-lg font-semibold">{it.t}</h3>
...
export function Simulator() {
  ...
  const calc = async (e?: React.SyntheticEvent) => {
    ...
    setError(null);
    try {
      const res = await lookup({ data: { name: form.name, city: form.city, category: form.category } });
      if (res.rating === null && res.reviewCount === null) {
        setError("Não encontramos esse negócio no Google Maps. Confira o nome e a cidade.");
        return;
      }
      setResult({
        score: res.score,
      ...
      const msg = err instanceof Error ? err.message : "Falha ao calcular o score";
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <section id="simulator" className="border-b border-border py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid items-start gap-12 lg:grid-cols-2">
          <div>
            <div className="text-xs uppercase tracking-wider text-accent">{t.simulator.eyebrow}</div>
            <h2 className="font-display mt-3 text-balance text-3xl font-semibold tracking-tight md:text-4xl">{t.simulator.title}</h2>
            <p className="mt-4 text-muted-foreground">{t.simulator.subtitle}</p>
          </div>

          <div className="rounded-2xl border border-border bg-card p-6 md:p-8" style={{ boxShadow: "var(--shadow-elegant)" }}>
            {!result ? (
              <div className="space-y-4" onKeyDown={(e) => { if (e.key === "Enter") calc(e); }}>
                ...
                <button type="button" onClick={calc} disabled={!form.name || !form.city || loading} className="flex w-full items-center justify-center gap-2 rounded-lg bg-foreground py-2.5 text-sm font-medium text-background transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">
                  {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Analisando Google…</> : t.simulator.cta}
                </button>
                <p className="text-[11px] text-muted-foreground">
                  Buscamos dados reais no Google Maps via IA e calculamos seu score em segundos.
                </p>
              </div>
            ) : (
              <motion.div ref={resultRef} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
                <div className="text-xs uppercase tracking-wider text-muted-foreground">{t.simulator.resultTitle}</div>
                <div className="text-sm text-muted-foreground">
                  {t.simulator.resultSub} <span className="font-medium text-foreground">{result.name || form.name}</span>
                </div>
                <div className="flex items-end gap-4">
                  <div className="font-display text-6xl font-semibold tabular-nums">{result.score}</div>
                  <div className="mb-2">
                    <div className="flex items-center gap-1 text-amber-500">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={`h-4 w-4 ${i < Math.round(result.score / 20) ? "fill-current" : ""}`} />
                      ))}
                    </div>
                    <div className="mt-1 text-xs text-muted-foreground">Reputation Score · 0–100</div>
                  </div>
                </div>
                {(result.rating !== null || result.reviewCount !== null) && (
                  <div className="grid grid-cols-2 gap-2 rounded-lg border border-border bg-muted/30 p-3 text-xs">
                    ...
                    <div>
                      ...
                      <div className="font-mono text-sm text-foreground">
                        {result.reviewCount !== null ? result.reviewCount.toLocaleString() : "—"}
                      </div>
                    </div>
                  </div>
                )}
                <div className="space-y-2">
                  <Bar label={t.simulator.benchmark} value={result.bench} color="oklch(0.7 0.02 260)" />
                  <Bar label={result.name || form.name || "You"} value={result.score} color="oklch(0.6 0.18 265)" />
                </div>
                <div className="rounded-lg border border-border bg-muted/40 p-3 text-xs text-muted-foreground">
                  <span className="font-medium text-foreground">{t.simulator.gap}:</span>{" "}
                  {result.score < result.bench ? `−${(result.bench - result.score).toFixed(1)} pts` : `+${(result.score - result.bench).toFixed(1)} pts`}
                </div>
                <button type="button" onClick={() => { setResult(null); setError(null); }} className="w-full rounded-lg border border-border py-2 text-xs text-muted-foreground hover:text-foreground">
                  Calcular outro negócio
                </button>
                <Link to="/sign-up" className="block w-full rounded-lg bg-foreground py-2.5 text-center text-sm font-medium text-background transition hover:opacity-90">
                  {t.simulator.claim}
                </Link>
              </motion.div>
            )}
    ...
    <section id="pricing" className="border-b border-border py-20 md:py-28">
      <div className="mx-auto max-w-7xl px-6">
        <div className="mx-auto max-w-2xl text-center">
          <div className="text-xs uppercase tracking-wider text-accent">{t.pricing.eyebrow}</div>
          <h2 className="font-display mt-3 text-balance text-3xl font-semibold tracking-tight md:text-4xl">{t.pricing.title}</h2>
            ...
            return (
              <div
                ...
                className={`relative rounded-2xl border bg-card p-6 transition ${
                  p.highlight ? "border-foreground/40 shadow-xl" : "border-border"
                }`}
              ...
              >
                ...
                <div className="mt-5 flex items-baseline gap-1">
                  ...
                  {p.brl > 0 && <span className="text-xs text-muted-foreground">{t.pricing.perMonth}</span>}
                </div>
                <Link
                  to="/sign-up"
        ...
        <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-dashed border-border bg-muted/30 p-5">
          <div>
            <div className="font-display text-sm font-semibold">{t.pricing.addonTitle}</div>
            <p className="text-xs text-muted-foreground">{t.pricing.addonDesc}</p>
          </div>
          <div className="font-display text-xl font-semibold tabular-nums">{format(19.9, 4)}{t.pricing.perMonth}</div>
        </div>
      </div>
    </section>
  );
    ...
    <section className="border-b border-border py-20 md:py-28">
      <div className="mx-auto max-w-5xl px-6">
        <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-card via-card to-muted/60 p-10 text-center md:p-16">
          ...
          <div className="relative">
            <h2 className="font-display mx-auto max-w-2xl text-balance text-3xl font-semibold tracking-tight md:text-5xl">{t.cta.title}</h2>
            <p className="mx-auto mt-4 max-w-xl text-muted-foreground">{t.cta.subtitle}</p>
            <div className="mt-7 flex flex-wrap items-center justify-center gap-3">
              <Link to="/sign-up" className="inline-flex items-center gap-2 rounded-lg bg-foreground px-5 py-2.5 text-sm font-medium text-background transition hover:opacity-90">
                {t.cta.primary} <ArrowRight className="h-4 w-4" />
              </Link>
              <a href="mailto:hello@branchly.com" className="inline-flex items-center gap-2 rounded-lg border border-border bg-background/50 px-5 py-2.5 text-sm font-medium transition hover:bg-muted">
                {t.cta.secondary}
              </a>
            </div>
          </div>
    ...
    <footer className="py-12">
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid grid-cols-2 gap-8 md:grid-cols-4">
          ...
          <FooterCol title={t.footer.product} links={["Features", "Pricing", "Changelog", "Roadmap"]} />
          <FooterCol title={t.footer.company} links={["About", "Customers", "Careers", "Contact"]} />
        </div>
        <div className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-border pt-6 text-xs text-muted-foreground">
          <div>© {new Date().getFullYear()} Branchly. {t.footer.rights}</div>
