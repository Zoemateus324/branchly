    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogTitle>
            Faça upgrade para {PLAN_LABEL[suggestedPlan]}
          </DialogTitle>
          <DialogDescription>
            Seu plano atual ({PLAN_LABEL[currentPlan]}) não permite{" "}
            {FEATURE_LABELS[feature]}. Faça upgrade para liberar este recurso.
          </DialogDescription>
        </DialogHeader>
        <ul className="space-y-2 rounded-lg border border-border bg-muted/30 p-4 text-sm">
          ...
          <Item ok={limits.competitors > 0}>
            {limits.competitors >= 9999
              ? "Concorrentes ilimitados"
              : `${limits.competitors} concorrentes`}
          </Item>
          <Item ok={limits.aiReplies > 0}>
            {limits.aiReplies >= 9999
              ? "Respostas IA ilimitadas"
              : `${limits.aiReplies} respostas IA/mês`}
          </Item>
          <Item ok={limits.reports > 0}>
            {limits.reports >= 9999
              ? "Relatórios ilimitados"
              : `${limits.reports} relatórios/mês`}
          </Item>
        </ul>
...
function Item({ children, ok }: { children: React.ReactNode; ok: boolean }) {
  return (
    <li
      className={`flex items-center gap-2 ${ok ? "text-foreground" : "text-muted-foreground line-through"}`}
    >
