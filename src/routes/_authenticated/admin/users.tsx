...
function AdminUsers() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      list({
        data: { limit, offset: page * limit, query: query || undefined },
      }),
  });
  ...
  const overrideMut = useMutation({
    mutationFn: (input: {
      email: string;
      plan: "free" | "starter" | "pro" | "premium" | null;
  ...
  const banMut = useMutation({
    mutationFn: (input: { userId: string; ban: boolean }) =>
      ban({ data: input }),
    onSuccess: () => {
                    ...
                    <tr key={u.id} className="border-b last:border-0">
                      ...
                      <td className="p-2">
                        <Button
                          ...
                          onClick={() =>
                            banMut.mutate({ userId: u.id, ban: !u.banned })
                          }
                        >
